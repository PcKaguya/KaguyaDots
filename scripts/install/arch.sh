...
# Clone dotfiles
clone_dotfiles() {
  gum style --border double --padding "1 2" --border-foreground 212 "Cloning Kaguya Dotfiles"
...
install_packages() {
  ...
  paru | yay)
    ...
    set +e
    $PACKAGE_MANAGER -S --needed --noconfirm "${INSTALL_PACKAGES[@]}" 2>&1 | tee /tmp/kaguya_install.log
    local install_exit_code=$?
    set -e

    # Check if all packages were already up to date
    if grep -q "there is nothing to do" /tmp/kaguya_install.log 2>/dev/null || \
       grep -q "is up to date -- skipping" /tmp/kaguya_install.log 2>/dev/null; then
...
install_shell_scripts() {
  ...
  local scripts_dir="$KAGUYADIR/config/local-bin"
  ...
  # Install kaguya.sh
  if [ -f "$scripts_dir/kaguya.sh" ]; then
    echo "Installing kaguya script..." "slide"
    cp "$scripts_dir/kaguya.sh" "$HOME/.local/bin/kaguya"
    chmod +x "$HOME/.local/bin/kaguya"
    echo "✓ kaguya installed to ~/.local/bin/kaguya" "slide"
  else
    gum style --foreground 220 "⚠ kaguya.sh not found at $scripts_dir/kaguya.sh"
  fi
...
# Create Kaguya configuration file
create_kaguya_config() {
  gum style --border double --padding "1 2" --border-foreground 212 "Creating Kaguya Configuration"
  ...
  if command -v curl &>/dev/null; then
    local remote_version=$(curl -s "https://raw.githubusercontent.com/Nurysso/Kaguya/main/version.txt" 2>/dev/null || echo "")
  ...
  # Create kaguya.toml
  cat >"$config_file" <<EOF
# Kaguya Dotfiles Configuration
# This file manages your Kaguya installation settings
...
EOF
  ...
  gum style --foreground 82 "✓ Kaguya config created at: $config_file"
...
setup_wallpapers() {
  ...
  if gum confirm "Download full wallpaper repository?"; then
    ...
    local default_dir="$wallpaper_dir/kaguya-default"
    mkdir -p "$default_dir"
...
main() {
  ...
  if ! gum confirm "Do you want to proceed with Kaguya installation?"; then
    clear
    gum style \
      --foreground 220 \
      --border normal \
