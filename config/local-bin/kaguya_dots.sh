...
# KaguyaDots Dotfiles Manager
# Manages updates, configuration, and theme settings

set -e

CONFIG_FILE="$HOME/.config/kaguyadots/kaguyadots.toml"
KAGUYADOTS_DIR="$HOME/KaguyaDots"
REPO_URL="https://github.com/PcKaguya/KaguyaDots.git"
UPDATE_SCRIPT_URL="https://raw.githubusercontent.com/PcKaguya/KaguyaDots/main/update.sh"

# Ensure config directory exists
mkdir -p "$HOME/.config/kaguyadots"
...
if [ ! -f "$CONFIG_FILE" ]; then
  gum style --foreground 196 "Error: KaguyaDots config not found!"
...
check_updates() {
  ...
  if [ -z "$remote_version" ]; then
    ...
    send_notification "KaguyaDots Update Check" "Failed to reach GitHub (skill issue?)" "normal"
    return 1
  fi
  ...
  if [ "$current_num" != "$remote_num" ]; then
    ...
    send_notification "KaguyaDots Update Check" "Already on latest version. Nothing to do here." "low"
    return 1
  fi
}
  ...
  if [ -n "$remote_data" ]; then
    ...
    if [ "$remote_version" != "$local_num" ]; then
      ...
      if printf '%s\n%s\n' "$remote_version" "$local_num" | sort -V | head -n1 | grep -q "^$local_num$"; then
        ...
        send_notification "KaguyaDots Update Available" \
          "New version detected!\nRemote: $remote_data\nLocal: $local_version\nRun: kaguyadots update" "critical"
      else
        ...
        send_notification "KaguyaDots Version Mismatch" \
...
# Update KaguyaDots using update.sh from GitHub
update_kaguyadots() {
  # Check network first
  if ! check_network "true"; then
    ...
    send_notification "KaguyaDots Update Failed" "No network = no update. Pretty simple." "critical"
    exit 1
  fi

  # Download update.sh
  local update_script="/tmp/kaguyadots-update.sh"
  ...
  if [ -f "$update_script" ]; then
    ...
    send_notification "KaguyaDots Updated" \
    ...
    send_notification "KaguyaDots Update Failed" "Couldn't download update. Try again later?" "critical"
    exit 1
  fi
}
...
toggle_theme() {
  ...
  if [ "$current_mode" = "dynamic" ]; then
    ...
    send_notification "KaguyaDots Theme Changed" \
...
show_info() {
  ...
  gum style --foreground 220 "  kaguyadots check      Check for updates"
  gum style --foreground 220 "  kaguyadots update     Update dotfiles"
  gum style --foreground 220 "  kaguyadots theme      Toggle theme mode"
  gum style --foreground 220 "  kaguyadots network    Check network"
}
...
show_help() {
  gum style \
    --foreground 212 --border-foreground 212 --border double \
    --align center --width 60 --margin "1 2" --padding "1 2" \
    'KAGUYADOTS DOTFILES HELPER'
  echo ""
  gum style --foreground 82 --bold "Usage:"
  gum style --foreground 220 "  kaguyadots [command]"
  ...
  gum style --foreground 220 "  KaguyaDots startup    # Morning ritual"
  gum style --foreground 220 "  KaguyaDots check      # See if updates exist"
  gum style --foreground 220 "  KaguyaDots update     # YOLO"
  gum style --foreground 220 "  KaguyaDots theme      # Identity crisis mode"
...
show_term() {
  ...
  if [ -z "$terminal" ]; then
    ...
    notify-send "KaguyaDots Error" "Terminal preference not found in config" "critical"
    return 1
  fi
...
update)
  if check_updates; then
    ...
    if gum confirm "Actually run the update?"; then
      ...
      send_notification "KaguyaDots Update" "Update cancelled. Coward." "low"
    fi
  fi
  ;;
theme)
...
*)
  ...
  gum style --foreground 220 "That's not a thing. Try 'kaguyadots help'"
  echo ""
  show_help
  send_notification "KaguyaDots Error" "Invalid command. RTFM." "critical"
  exit 1
  ;;
esac
