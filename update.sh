...
# KaguyaDots Update Script
# Description: Updates KaguyaDots dotfiles with backup and configuration preservation
...
VERSION_FILE="$KAGUYADOTS_DIR/version.txt"
...
# Check if KaguyaDots is installed
check_kaguyadots_installed() {
  if [ ! -f "$CONFIG_FILE" ]; then
    gum style --foreground 196 --bold "❌ Error: KaguyaDots is not installed!"
...
show_update_warning() {
  gum style --foreground 220 "Update available!"
  gum style --foreground 196 --bold "Update warning:"
  gum style --foreground 220 "This update may contain breaking changes or require manual intervention."
  gum style --foreground 220 "Please review the release notes before proceeding."
...
install_shell_scripts() {
  ...
  if [ -f "$scripts_dir/kaguyadots.sh" ]; then
    echo "Installing kaguyadots script..." "slide"
    cp "$scripts_dir/kaguyadots.sh" "$HOME/.local/bin/kaguyadots"
    chmod +x "$HOME/.local/bin/kaguyadots"
    echo "✓ kaguyadots installed to ~/.local/bin/kaguyadots" "slide"
  else
...
# Update KaguyaDots config file with new version
update_kaguyadots_config() {
  gum style --border double --padding "1 2" --border-foreground 212 "Updating KaguyaDots Configuration"
  ...
  gum style --foreground 82 "✓ KaguyaDots config updated"
...
show_completion_message() {
  local backup_path=$(cat "$HOME/.cache/kaguyadots_last_backup.txt" 2>/dev/null || echo "")

  echo ""
  gum style --border double --padding "1 2" --border-foreground 82 "✓ Update Complete!"

  gum style --foreground 82 --bold "KaguyaDots has been successfully updated!"
  echo ""
  ...
  echo ""
  gum style --foreground 82 "Thank you for using KaguyaDots! 🌙"
}
# Setup wallpapers
setup_wallpapers() {
  ...
  if gum confirm "Download full wallpaper repository?"; then
    ...
    local lock_screen_url="https://raw.githubusercontent.com/Nurysso/Freya/main/walls/kaguyadots-default/lock-screen.png"
    local wallpaper_url="https://raw.githubusercontent.com/Nurysso/Freya/main/walls/kaguyadots-default/wallpaper.png"
    local success=0
