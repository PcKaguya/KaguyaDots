...
# Global Variables
KAGUYADIR="$HOME/KaguyaDots"
KAGUYAAPPSDIR="$HOME/KaguyaDots/apps"
...
clone_dotfiles() {
  ...
  if ! git clone --depth 1 "$REPO_URL" "$KAGUYADIR"; then
...
backup_config() {
  ...
  local backup_dir="$HOME/.cache/kaguya-backup/kaguya-$timestamp"

  # List of config directories to check (excluding shell rc files)
   local config_dirs=(
 "alacritty"  "cava" "fastfetch"  "foot"     "gtk-3.0"  "kaguya"  "kitty"    "quickshell"  "starship" "wallust" "waypaper" "zsh" "bash"  "fish" "ghostty" "gtk-4.0" "hypr" "matugen" "rofi" "swaync" "waybar" "wlogout"
  )
  ...
  if [ "$backed_up" = true ]; then
    ...
    echo "$backup_dir" > "$HOME/.cache/kaguya_last_backup.txt"
  else
...
install_packages() {
  ...
  set +e
  sudo dnf install -y --skip-broken --skip-unavailable --allowerasing "${INSTALL_PACKAGES[@]}" 2>&1 | tee /tmp/kaguya_install.log
  ...
  #   local failed_log="$HOME/kaguya_failed_packages.txt"
...
move_config() {
  ...
  if [ ! -d "$KAGUYADIR/config" ]; then
    gum style --foreground 196 "Error: Config directory not found at $KAGUYADIR/config"
    exit 1
  fi
  ...
  # Copy all config directories except shell rc files and kaguya.sh
  for item in "$KAGUYADIR/config"/*; do
    if [ -d "$item" ]; then
  ...
  if [ -f "$KAGUYADIR/config/zshrc" ]; then
    gum style --foreground 82 "Installing .zshrc..."
    cp "$KAGUYADIR/config/zshrc" "$HOME/.zshrc"
    gum style --foreground 82 "✓ ZSH config installed"
  else
    gum style --foreground 220 "⚠ zshrc not found in config directory"
  fi

  if [ -f "$KAGUYADIR/config/bashrc" ]; then
    gum style --foreground 82 "Installing .bashrc..."
    cp "$KAGUYADIR/config/bashrc" "$HOME/.bashrc"
    gum style --foreground 82 "✓ BASH config installed"
  else
    gum style --foreground 220 "⚠ bashrc not found in config directory"
  fi

  # Install kaguya CLI tool
  if [ -f "$KAGUYADIR/config/kaguya.sh" ]; then
    gum style --foreground 82 "Installing kaguya CLI tool..."
    cp "$KAGUYADIR/config/kaguya.sh" "$HOME/.local/bin/kaguya"
    chmod +x "$HOME/.local/bin/kaguya"
    gum style --foreground 82 "✓ kaguya command installed"
  else
    gum style --foreground 220 "⚠ kaguya.sh not found in config directory"
  fi

  # Install apps from apps directory
  install_app "Pulse" "$KAGUYAAPPSDIR/Pulse/build/bin/Pulse"
  install_app "KaguyaDots-Settings" "$KAGUYAAPPSDIR/KaguyaDots-Help/build/bin/KaguyaDots-Settings"
...
build_quickApps(){
  ...
  cat >~/.config/kaguya/quickapps.conf <<EOF
...
# Create KaguyaDots configuration file
create_kaguya_config() {
  gum style --border double --padding "1 2" --border-foreground 212 "Creating KaguyaDots Configuration"

  local config_dir="$HOME/.config/kaguya"
  local config_file="$config_dir/kaguya.toml"
  local version="1.0.0"
  local install_date=$(date +%Y-%m-%d)

  # Try to get version from remote
  if command -v curl &>/dev/null; then
    local remote_version=$(curl -s "https://raw.githubusercontent.com/Nurysso/KaguyaDots/main/version.txt" 2>/dev/null || echo "")
  ...
  # Create kaguya.toml
  cat >"$config_file" <<EOF
# KaguyaDots Dotfiles Configuration
# This file manages your KaguyaDots installation settings
...
EOF
  ...
  gum style --foreground 82 "✓ KaguyaDots config created at: $config_file"
...
setup_Waybar() {
  ...
  # Prefer canonical 'kaguyadots' color file, but fall back to older paths or repo defaults if necessary.
...
setup_wallpapers() {
  ...
  if gum confirm "Download full wallpaper repository?"; then
    ...
    local lock_screen_url="https://raw.githubusercontent.com/Nurysso/Freya/main/walls/kaguya-default/lock-screen.png"
    local wallpaper_url="https://raw.githubusercontent.com/Nurysso/Freya/main/walls/kaguya-default/wallpaper.png"
    local success=0
...
main() {
  ...
  gum style \
    ...
    'KAGUYADOTS DOTFILES' 'Hyprland Configuration Installer' ''

  # Confirm installation
  if ! gum confirm "Do you want to proceed with KaguyaDots installation?"; then
  ...
  if [ ${#FAILED_PACKAGES[@]} -gt 0 ]; then
    ...
    gum style --foreground 220 "Check ~/kaguya_failed_packages.txt for details"
    echo ""
  else
